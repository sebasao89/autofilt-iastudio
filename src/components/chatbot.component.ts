...
@Component({
  ...
  template: `
    ...
    <div 
      ...
      <!-- Header -->
      <div class="bg-[#2a2420] p-4 border-b border-[#4d423d] flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-green-400 flex items-center gap-1">
              <span class="size-1.5 rounded-full bg-green-400"></span> {{ ts.text().chat.online }}
            </p>
          </div>
        </div>
        <button (click)="toggleChat()" class="text-text-muted hover:text-white transition-colors">
          <span class="material-symbols-outlined">close</span>
        </button>
      </div>

      <!-- Messages Area -->
      <div #scrollContainer class="flex-1 overflow-y-auto p-4 space-y-4 bg-[#1a1716]">
        ...
        @if (isLoading()) {
          <div class="flex justify-start">
            <div class="bg-[#332c29] px-4 py-3 rounded-2xl rounded-tl-sm border border-[#4d423d] flex gap-1">
              <span class="size-2 rounded-full bg-primary/50 animate-bounce" style="animation-delay: 0ms"></span>
              <span class="size-2 rounded-full bg-primary/50 animate-bounce" style="animation-delay: 150ms"></span>
              <span class="size-2 rounded-full bg-primary/50 animate-bounce" style="animation-delay: 300ms"></span>
            </div>
          </div>
        }
      </div>
...
export class ChatbotComponent {
  ...
  constructor() {
    ...
    effect(() => {
        ...
        if (this.messages().length === 0) {
             this.messages.set([{role: 'model', text: this.ts.text().chat.initial}]);
        }
  ...
  async sendMessage() {
    ...
    const response = await this.aiService.chat(userMsg, history, this.ts.currentLang());
    
    this.messages.update(msgs => [...msgs, {role: 'model', text: response || 'Sorry, I missed that.'}]);
